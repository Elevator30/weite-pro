// 附表HTML生成（每个附表单独一页A4竖版）- 按纸质版PDF格式复刻
function buildSingleAttachHTML(task, dateStr, attNum) {
  ...
  if (attNum === 1) {
    ...
    var dataRows = [];
    // 轿门1、2
    dataRows.push({label: '轿门1', isCar: true, idx: 0});
    dataRows.push({label: '轿门2', isCar: true, idx: 1});
    // 层门
    var laygateCount = Math.max(laygateList.length, 10);
    for (var li = 0; li < laygateCount; li++) {
      var layerName = (laygateList[li] && laygateList[li].name) ? laygateList[li].name : ((li+1) + '层');
      dataRows.push({label: layerName, isCar: false, idx: li});
    }
    ...
    h += '<td style="border:1px solid #000;padding:2px;width:38%;">项目</td>';
    h += '<td style="border:1px solid #000;padding:2px;width:8%;">状态</td>';
    h += '<td style="border:1px solid #000;padding:2px;width:16%;">上端站平层时</td>';
    h += '<td style="border:1px solid #000;padding:2px;width:16%;">对重完全压在缓冲器上时</td>';
    h += '<td style="border:1px solid #000;padding:2px;width:16%;">检验结果</td>';
    h += '</tr>';
    ...
    h += '<td style="border:1px solid #000;padding:2px;width:38%;">项目</td>';
    h += '<td style="border:1px solid #000;padding:2px;width:8%;">状态</td>';
    h += '<td style="border:1px solid #000;padding:2px;width:16%;">下端站平层时</td>';
    h += '<td style="border:1px solid #000;padding:2px;width:16%;">轿厢完全压在缓冲器上时</td>';
    h += '<td style="border:1px solid #000;padding:2px;width:16%;">检验结果</td>';
    h += '</tr>';
